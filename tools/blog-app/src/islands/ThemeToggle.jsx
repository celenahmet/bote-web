...
const LABEL = { light: 'Tema: açık', dark: 'Tema: koyu' };

// Varsayilan acik tema; koyu tema secilirse tarayicida hatirlanir.
function apply(mode) {
  const el = document.documentElement;
  if (mode === 'dark') el.dataset.theme = 'dark';
  else delete el.dataset.theme;
  try { mode === 'dark' ? localStorage.setItem('bote-theme', 'dark') : localStorage.removeItem('bote-theme'); } catch {}
}
...
export default function ThemeToggle() {
  ...
  return (
    <button type="button" className="icon-btn" onClick={next} aria-label={mode === 'dark' ? 'Açık temaya geç' : 'Koyu temaya geç'} title={LABEL[mode]}>
      {mode === 'light' && (
